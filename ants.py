...
class HungryAnt(Ant):# The HungryAnt Class
    ...
    implemented = True   # Change to True to view in the GUI
    def __init__(self, health=1):
        """Create an Ant with a HEALTH & CHEW DURATION quantity."""
        super().__init__(health)
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self, gamestate): 
        try:
            ...
            if gamestate.queen:
                raise DuplicateQueensException("you cannot create a second QueenAnt")
        ...
        except DuplicateQueensException as d:
            print(d)        
    ...
    def action(self, gamestate):
        ...
        if self.imposter: 
            self.reduce_health(self.health) # Kill the imposter immediately 
        else:
            pos = self.place.exit
            while pos:
                if pos.ant is not None:
                    if not pos.ant.buffed:
                        pos.ant.buffed = True 
                        pos.ant.buff()
                    if pos.ant.is_container and pos.ant.ant_contained:
                        # the pos.ant.ant_contained may change
                        if not pos.ant.ant_contained.buffed:
                            pos.ant.ant_contained.buff()
                            pos.ant.ant_contained.buffed = True
                pos = pos.exit
